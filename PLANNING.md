...
*协议层：*
- 定义 FSP 协议 v1.0（含组件树、属性、样式、数据源、基础表达式）
...
- 拖拽画布（组件拖入、排序、选中、删除）
- 属性面板（文本、数字、布尔、枚举、颜色、图片 6 种 Setter）
- 画布内实时预览

*内置组件（8 个）：*
- 基础：Page、View、Text、Image、Button、Input
- 数据驱动：**List（列表循环）**、**Card（数据卡片）**

*数据绑定（简化版）：*
- 数据源面板：可配置 API 数据源（URL + Method + 参数）
- 基础表达式绑定：`{{$ds.数据源ID.data.字段名}}`
- List 组件支持绑定数组数据源，自动循环渲染子组件
- Card 组件支持绑定对象数据源，映射字段到子元素
- 编辑器内用 Mock 数据预览绑定效果

*Taro 代码生成：*
- `codegen-core`：协议解析 → 中间表示（IR）
- `codegen-taro`：IR → Taro React TSX + SCSS
- 生成完整可运行的 Taro 项目（含 API 请求、状态管理）
- 编辑器内代码预览面板（Monaco Editor）
- 项目下载（zip 包）
